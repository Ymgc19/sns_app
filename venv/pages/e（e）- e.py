...
st.table(df.T)
st.subheader('論文情報の変更')
koumoku = ['名前', '読んだ日', 'タイトル', '著者名', '出版年', 'キーワード', '分野', '概要', '手法', '評価']
options = ['GIS', '経済', '農業', '環境', '階層', '教育', '家族', '政治', '思想', '心理', '統計', 'メディア', 'その他']
with st.form(key = '変更項目の選択'):
    selected_koumoku = st.selectbox('変更する項目', koumoku)
    koumoku_btn = st.form_submit_button('SUBMIT')
if koumoku_btn:
        if selected_koumoku == '分野':
                henkou = st.multiselect('変更後', options)
                if st.button('入力完了'):
                    df.at[0, selected_koumoku] = henkou
                    df.to_csv(path1)
                    st.text('変更を受け取りました！')
        elif selected_koumoku == '読んだ日':
                henkou = st.date_input('読んだ日', datetime.date.today())
                if st.button('入力完了'):
                    df.at[0, selected_koumoku] = henkou
                    df.to_csv(path1)
                    st.text('変更を受け取りました！')
